...
async function getSchoolInfo(schoolID) {
  if (!schoolID) {
    console.log('School ID is not provided.');
    return null;
  }
  ...
  try {
    ...
    if (querySnapshot.empty) {
      console.log(`No school found for ID: ${schoolID}`);
      return null;
    } else {
  ...
  for (const shiftKey in shiftsData) {
    if (shiftsData.hasOwnProperty(shiftKey)) {
      ...
      for (const shiftInfo of shiftsArray) {
        ...
        if (!schoolAddress) {
          console.log("No school address for shift:", shiftKey);
    ...
    for (const shift of shiftsArray) {
      ...
      for (const availableTeacher of shift.availableEmployees) {
        ...
        if (teacherRoute) {
          ...
          console.log(durationScore, distanceScore)
          // Combine scores
...
async function assignTeachersToShifts(scoredShifts) {
  ...
  console.log("greedyAssignment", formattedGreedyAssignment)
  console.log(greedyAverage)
...
async function assignGreedy(scoredShifts) {
 ...
 async function POST(req, res) {
  ...
  console.log(`getShiftsWithinTimePeriod POST Recieved: startDateTimestamp: ${startDateTimestamp}, endDateTimestamp: ${endDateTimestamp}`)
  ...
  try {
      ...
      const availableTeachersForShifts = findAvailableTeachersForShifts(shifts, availabilityData);
      const availableTeachersForShiftsWithTeacherData = await addTeacherDataToShifts(availableTeachersForShifts, allTeachers);
      const updatedShiftsWithProgramInfo = await addProgramInfoToShifts(availableTeachersForShiftsWithTeacherData);
      const updatedShiftsWithSchoolInfo = await addSchoolInfoToShifts(updatedShiftsWithProgramInfo);
      const withTripDetails = await fetchAndAddRouteInfo(updatedShiftsWithSchoolInfo);

      const simplifiedShifts = await simplifyShiftData(withTripDetails);
      const scoredShifts = await scoreTeacherForShift(simplifiedShifts);
      const assignedTeachers = await assignTeachersToShifts(scoredShifts);

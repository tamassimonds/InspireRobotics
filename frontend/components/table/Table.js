...
function isDateKey(key) {
  // Add logic to determine if a key corresponds to a date field
  // For example, keys that include 'date' might be date fields
  return key.toLowerCase().includes('date');
}

function descendingComparator(a, b, orderBy) {
  if (isDateKey(orderBy)) {
    ...
    // Handle non-date fields as before
...
export default function TableSortAndSelection({fields = [], data = [], name = "", handleRowSelected, 
defaultSortKey = "date", defaultSortDir = "asc", rowsPerPageProp=10}) {
  const [order, setOrder] = React.useState(defaultSortDir);
  const [orderBy, setOrderBy] = React.useState(defaultSortKey);
  const [selected, setSelected] = React.useState([]);
  const [page, setPage] = React.useState(0);
  const [tableWidthStyle, setTableWidthStyle] = React.useState({});

  React.useEffect(() => {
    // Dynamically adjust the table width based on the number of columns
    const columnWidth = fields.length === 1 ? '100%' : 'auto';
    setTableWidthStyle({
      '& thead th': {
        width: columnWidth,
      },
      ...
      <Sheet variant="outlined" sx={{ width: '100%', boxShadow: 'sm', borderRadius: 'sm' }}>
        ...
        <Table
          ...
          sx={{
            ...
            ...tableWidthStyle, // Apply the dynamically calculated style
            '& tr': {
              textAlign: 'center', // Adjust based on your requirements
            },
            '& tr > *': {
              textAlign: 'left', // Aligns text to the left for all cells, adjust as necessary
            },
            '& tr > *:nth-child(n+1)': {
              textAlign: 'left', // Adjust based on your requirements
            },
          }}
        >
        <EnhancedTableHead
            ...
            {stableSort(rows, getComparator(order, orderBy))
              ...
              .map((row, index) => {
                ...
                return (
                  ...
                  >
                    {headCells.map((cell) => {
                        // Check if the cell does not have a hidden property or if it's not set to true

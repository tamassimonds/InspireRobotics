...
export default function BioCard({employeeID}) {
  const [employeeData, setEmployeeData] = useState()
  const fetchEmployeeData = async () => {
    const fetchedEmployeeData = await getEmployeeWithID(employeeID);
  ...
  return (
    ...
    >
      <CardContent sx={{ alignItems: 'center', textAlign: 'center' }}>
        <Avatar src={employeeData?.profileImage ?? "https://www.iprcenter.gov/image-repository/blank-``file-picture.png/@@images/image.png"} sx={{ '--Avatar-size': '12rem' }} />
        <Chip
          size="sm"
          variant="soft"
          color="primary"
        ...
        <Typography level="title-lg">{employeeData?.name ?? ""}</Typography>
        <Typography level="body-sm" sx={{  }}>
          {employeeData?.universityCourse ?? ""}
        </Typography>
